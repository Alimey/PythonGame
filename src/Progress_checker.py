...
class ProgressChecker:
  ...
  объект класса Window.
    ...
    __next_round() - переход к следующему раунду
  ...
  def on_clicked(self, i):
    ...
    """
    buttons = self.__buttons.get_buttons()
    ...
    if self.round == AMOUNT_OF_ROUNDS:
      ...
      return
    self.__root.bind('<Key>', lambda event: self.__next_round())

  def __next_round(self):
    """Переход к сделующему раунду"""
    self.__root.unbind('<Key>')
    self.__text_displayer.add_message(ROUND_MESSAGES[self.round], True)
  ...
  def __final(self):
    ...
    self.__text_displayer.add_message("\n\n 'Нажмите любую клавишу для закрытия окна'", False)
    self.__root.unbind('<Key>')
